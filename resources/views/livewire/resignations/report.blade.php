...
<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">Resignation report</flux:heading>
            <flux:subheading>Turnover analysis and resignation trends</flux:subheading>
        </div>
        <flux:button href="{{ route('resignations.index') }}" icon="arrow-left" variant="ghost">
            Back to list
        </flux:button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <flux:card class="text-center">
            <p class="text-sm text-zinc-500">Total resignations</p>
        ...
        <flux:card class="text-center">
            ...
            <p class="text-4xl font-semibold text-yellow-500 mt-1">{{ $inProgress }}</p>
        </flux:card>
    </div>
        ...
        <div class="flex items-end gap-3 h-32">
            ...
            @forelse($byMonth as $month => $count)
                <div class="flex flex-col items-center gap-1 flex-1">
                    ...
                    <div
                        class="w-full bg-teal-500 rounded-t-sm transition-all"
    ...
    {{-- Full list --}}
    <flux:card>
        <flux:heading size="sm" class="mb-4">All resignations</flux:heading>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    @forelse($resignations as $r)
                        <tr>
                            <td class="py-2 px-3">
                                ...
                                <div class="text-xs text-zinc-400">{{ $r->employee->staff_number }}</div>
                            </td>
                            <td class="py-2 px-3 text-zinc-500">{{ $r->employee->division_label }}</td>
                            <td class="py-2 px-3 text-zinc-500">{{ $r->employee->branch_label }}</td>
                            <td class="py-2 px-3 text-zinc-500">{{ $r->resignation_date->format('d M Y') }}</td>
                            <td class="py-2 px-3 text-zinc-500">{{ $r->reason_label }}</td>
                            <td class="py-2 px-3">
                                <flux:badge color="{{ $r->status_color }}" size="sm">
                                    {{ $r->status_label }}
                                </flux:badge>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-8 text-zinc-400">No resignations found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
