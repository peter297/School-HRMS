<div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                <thead class="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                        <th
                            class="px-4 py-3 text-left text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">
                            Employee</th>
                        <th
                            class="px-4 py-3 text-left text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">
                            Leave type</th>
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700 bg-white dark:bg-zinc-900">
                    @forelse($leaves as $leave)
                        <tr wire:key="{{ $leave->id }}"
                            ...
                            <td class="px-4 py-3">
                                <span
                                    class="{{ $leave->leaveType->is_paid ? 'text-green-600' : 'text-zinc-500' }} text-sm">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if ($leave->approval_stage === 'pending_hr')
                                    ...
                                    @if (in_array($leave->approval_stage, ['pending_line_manager', 'pending_hr', 'approved']))
                                        <flux:button wire:click="cancelLeave({{ $leave->id }})"
                                            wire:confirm="Cancel this leave? Balance will be restored if approved."
                                    ...
                                    @if (in_array($leave->status, ['pending', 'approved']))
                                        ...
                                        />
                                    @endif --}}
                                    <flux:button href="{{ route('leaves.show', $leave) }}" size="sm"
    ...
    <flux:modal wire:model="showRejectModal" class="max-w-md">
        ...
        <div class="mt-4 space-y-4">
            <flux:textarea wire:model="rejectionReason" label="Rejection reason"
                placeholder="State why this leave is being rejected…" rows="3" />
    ...
    {{-- HR Approve modal --}}
    <flux:modal wire:model="showHrApproveModal" class="max-w-md">
        <flux:heading size="lg">HR — Approve leave</flux:heading>
        <div class="mt-4">
            <flux:textarea wire:model="hrNotes" label="Notes (optional)" rows="2" />
        </div>
        <div class="flex justify-end gap-3 mt-6">
            <flux:button wire:click="$set('showHrApproveModal', false)" variant="ghost">Cancel</flux:button>
            <flux:button wire:click="confirmHrApprove" variant="primary" icon="check">Approve</flux:button>
        </div>
    </flux:modal>

    {{-- HR Reject modal --}}
    <flux:modal wire:model="showHrRejectModal" class="max-w-md">
        <flux:heading size="lg">HR — Reject leave</flux:heading>
        <div class="mt-4">
            <flux:textarea wire:model="hrNotes" label="Rejection reason" rows="3" required />
            @error('hrNotes')
                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div class="flex justify-end gap-3 mt-6">
            <flux:button wire:click="$set('showHrRejectModal', false)" variant="ghost">Cancel</flux:button>
            <flux:button wire:click="confirmHrReject" variant="danger" icon="x-mark">Reject</flux:button>
        </div>
    </flux:modal>

    {{-- Override modal --}}
    <flux:modal wire:model="showOverrideModal" class="max-w-md">
        <flux:heading size="lg">Override line manager rejection</flux:heading>
        <flux:subheading>This will approve the leave despite the line manager's rejection.</flux:subheading>
        <div class="mt-4">
            <flux:textarea wire:model="hrNotes" label="Override reason" rows="3" required />
            @error('hrNotes')
                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div class="flex justify-end gap-3 mt-6">
            <flux:button wire:click="$set('showOverrideModal', false)" variant="ghost">Cancel</flux:button>
            <flux:button wire:click="confirmOverride" variant="primary" icon="shield-check">Override & approve
            </flux:button>
        </div>
    </flux:modal>
</div>
