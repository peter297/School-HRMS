{{-- resources/views/livewire/terminations/index.blade.php --}}

<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">Terminations</flux:heading>
            <flux:subheading>Manage and track staff terminations</flux:subheading>
        </div>
        <div class="flex gap-2">
            <flux:button href="{{ route('terminations.report') }}" icon="chart-bar" variant="ghost">
                Report
            </flux:button>
            <flux:button href="{{ route('terminations.create') }}" icon="plus" variant="primary">
                Record termination
            </flux:button>
        </div>
    </div>
    ...
    @if($activeCount > 0)
        <flux:callout variant="danger" icon="exclamation-triangle" class="mb-4">
            <flux:callout.heading>{{ $activeCount }} active termination(s) in progress</flux:callout.heading>
    ...
    <flux:card class="mb-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <flux:select wire:model.live="filterReason">
                ...
                <flux:select.option value="performance">Performance</flux:select.option>
                <flux:select.option value="absenteeism">Absenteeism</flux:select.option>
                <flux:select.option value="other">Other</flux:select.option>
            </flux:select>
            <flux:select wire:model.live="filterStatus">
                <flux:select.option value="">All statuses</flux:select.option>
                <flux:select.option value="initiated">Initiated</flux:select.option>
    ...
    <flux:card>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-zinc-200 dark:border-zinc-700">
                        ...
                        <th class="text-left py-3 px-3 text-zinc-500 font-medium">Termination date</th>
                    ...
                    @forelse($terminations as $termination)
                        <tr wire:key="{{ $termination->id }}">
                            ...
                            <td class="py-3 px-3 text-zinc-500">
                                {{ $termination->last_working_day?->format('d M Y') ?? '—' }}
                            </td>
                            <td class="py-3 px-3">
                            ...
                            <td class="py-3 px-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-zinc-400 w-8">
                                        {{ $termination->completion_percentage }}%
                                    </span>
                                </div>
                            </td>
                            <td class="py-3 px-3">
                            ...
                            <td class="py-3 px-3">
                                <flux:button
                                    href="{{ route('terminations.show', $termination) }}"
        ...
        <div class="mt-4">{{ $terminations->links() }}</div>
    </flux:card>
</div>